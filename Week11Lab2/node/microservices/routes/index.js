...
// crUd   update an existing meeting
router.post('/updateMeeting', async function (req, res) {
  try {
    const result = await meetings.findOneAndUpdate(
      { _id: req.body._id },   // which document
      req.body,                // new values
      { new: true }            // return the updated doc
    );

    if (!result) {
      return res.status(404).json({ response: 'fail', message: 'Not found' });
    }

    return res.json({ response: 'success', meeting: result });
  } catch (err) {
...
// cruD   Should use DELETE . . . we'll fix this in Cloud next term
router.post('/deleteMeeting', async function (req, res, next) {
  try {
    const result = await meetings.deleteOne({ _id: req.body._id });
