...
export function GlobalContextProvider(props) {
    const [globals, setGlobals] = useState({ aString: 'init val', count: 0, hideHamMenu: true, Photos: [], dataLoaded: false })
    ...
    async function getAllPhotos() {
        ...
        setGlobals((previousGlobals) => { const newGlobals = JSON.parse(JSON.stringify(previousGlobals)); newGlobals.Photos = data.Photos; newGlobals.dataLoaded = true; return newGlobals })
    }
    ...
    async function editGlobalData(command) { // {cmd: someCommand, newVal: 'new text'}
        ...
        if (command.cmd == 'addPhoto') {
            ...
            setGlobals((previousGlobals) => {
                const newGlobals = JSON.parse(JSON.stringify(previousGlobals))
                newGlobals.Photos.push(command.newVal); return newGlobals
            })
        }
    }
